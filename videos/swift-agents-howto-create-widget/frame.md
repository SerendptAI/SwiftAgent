...
> Copied from `../swift-agents-howto-register`. One token is added — `--go`, the
> API Keys screen's green — because this is the first tutorial in the series to
> render that screen. See the palette note below.
...
| `--go` | `#00B37E` | The API Keys screen's `Generate` button and its one-time
  reveal panel. The app's real colour for that one screen; it appears in this
  tutorial only, and never as text. |
